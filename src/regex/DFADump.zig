...
pub fn stringify(self: DFA, alloc: std.mem.Allocator) ![]u8 {
    ...
    while (dfa_it.next()) |entry|{
        ...
        if (entry.value_ptr.accept_id != null) {
            const fmt = "d{d} [shape=\"doublecircle\", style=\"filled\", fillcolor=\"{s}\", label=\"r{d}\"]";
            try writer.print(fmt, .{i_src.?, cycler.getColor(entry.value_ptr.accept_id.?), entry.value_ptr.accept_id.?});
        }
